...
def get_text_input():
    text_input = st.text_area("記事を入力してください:", key="input", height=200)
...
def combine_text_segments(text_segments):
    # リスト内の各テキストを連結して一つの文字列にする
...
def main():
    ...
    if output_text:
        with response_container:
            st.markdown("## 入力した文章" + "("+ str(count_words(st.session_state.input_text)) + "文字)")
            st.write(st.session_state.input_text)
            st.markdown("## 要約された文章" + "(" + str(count_words(output_text)) + "文字)")
